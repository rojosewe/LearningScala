 ...
 * normal method. In this case theimplicit label has no effect. However, if such
 * a method misses arguments for its implicit parameters, such arguments will be
 * automatically provided. The actual arguments that are eligible to be passed
 * to an implicit parameter fall into two categories:
 *
 ...
 *
 * In the following example we define a method sum which computes the sum of a list of elements using the
 * monoid's add and unit operations. Please note that implicit values can not be
